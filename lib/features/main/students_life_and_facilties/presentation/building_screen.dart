              ...
              child: Form(
                ...
                child: Column(
                  children: [
                    ...
                    TextFormField(
                      decoration: const InputDecoration(labelText: 'Classroom Capacity'),
                      keyboardType: TextInputType.number,
                      onSaved: (val) => classroom = int.parse(val!),
                    ),
                    TextFormField(
                      decoration: const InputDecoration(labelText: 'Lab Capacity'),
                      keyboardType: TextInputType.number,
                    ...
                    TextFormField(
                      decoration: const InputDecoration(labelText: 'Year Constructed'),
                      keyboardType: TextInputType.number,
                      onSaved: (val) => yearConstructed = int.tryParse(val!) ?? yearConstructed,
                    ),
                    TextFormField(
                      decoration: const InputDecoration(labelText: 'Year Completed'),
                      keyboardType: TextInputType.number,
                      onSaved: (val) => yearCompleted = int.tryParse(val!) ?? yearCompleted,
                    ),
                ...
                if (formKey.currentState!.validate()) {
                  ...
                  setState(() {
                    allBuildings.add({
                      ...
                      'name': name,
                      'campus': selectedCampus == 'All' ? 'Main Campus' : selectedCampus,
                      'classroom': classroom,
                      'lab': lab,
...
void confirmDelete(Map<String, dynamic> bldg) {
  ...
  Widget buildHeader() {
    return Container(
      ...
      child: Row(
        children: const [
          Expanded(flex: 1, child: Text('Total Buildings', style: TextStyle(color: Colors.white))),
          Expanded(flex: 3, child: Text('Name', style: TextStyle(color: Colors.white))),
          Expanded(flex: 5, child: Text('Capacity', style: TextStyle(color: Colors.white))),
          Expanded(flex: 2, child: Text('Year Constructed', style: TextStyle(color: Colors.white))),
          Expanded(flex: 2, child: Text('Year Completed', style: TextStyle(color: Colors.white))),
          Expanded(flex: 2, child: Text('Doc.', style: TextStyle(color: Colors.white))),
          Expanded(flex: 1, child: Text('Action', style: TextStyle(color: Colors.white))),
        ],
      ),
    );
  }
            ...
            children: [
              ...
              Expanded(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("Classroom Capacity: ${bldg['classroom']} students per room"),
                    Text("Laboratory Capacity: ${bldg['lab']} computers/workstations"),
